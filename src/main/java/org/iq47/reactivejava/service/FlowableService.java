...
public class FlowableService {
    ...
    public FlowableService(DataProperties dataProperties, BackpressureSubscriber backpressureSubscriber, RecordGenerator recordGenerator) {
    ...
    @PostConstruct
    public void init() {
        backpressureSubscriber.setDelay(dataProperties.getInitialHandleDelayNs());
        inputSpeed = dataProperties.getInitialGenearteDelayNs();
        inputScheduler.scheduleWithFixedDelay(() -> queue.offer(recordGenerator.generateDeal()), inputSpeed, inputSpeed, TimeUnit.NANOSECONDS);
...
//                                System.out.println(Integer.MAX_VALUE - pool.getQueue().remainingCapacity());
                                if (Integer.MAX_VALUE - pool.getQueue().remainingCapacity() > dataProperties.getQueueScaleSize() &&
                                        Integer.MAX_VALUE - pool.getQueue().remainingCapacity() > lastRecordedQueueSize &&
                                        ChronoUnit.MILLIS.between(lastScaleStartTime, LocalDateTime.now()) > dataProperties.getScaleDelayMillis()) {
                                    lastScaleStartTime = LocalDateTime.now();
                                    int currentThreadsCount = threadsCount;
                                    int currentDelay = backpressureSubscriber.getDelay();
                                    if (currentThreadsCount < dataProperties.getMaxThreads()) {
                                        setThreadsCount(currentThreadsCount + 1);
                                        if (currentDelay > 1) {
                                            backpressureSubscriber.setDelay(Math.max(currentDelay - 1000, 1));
                                        }
